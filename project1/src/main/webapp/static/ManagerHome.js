...
function display(xhr){
	...
	data.forEach(function(item,index){
		...
		`<tr>
		      ...
		      <td>
		      <button onclick="approve(${item.reimbId})" type="button" class="btn btn-link">Approve</button>
...
function approve(id){
	...
	xhr.setRequestHeader("Authorization",token);
	let requestBody = `type=approve&reimbId=${id}`;
...
function reject(id){
	...
	xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xhr.setRequestHeader("Authorization",token);
	let requestBody = `type=reject&reimbId=${id}`;
	xhr.send(requestBody);
}
